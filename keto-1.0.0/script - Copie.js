function signup() {
    ...
    if (isFirstNameValid == false) {
        ...
        document.getElementById("firstNameError").style.color = "rgb(227, 142, 142)";
        isFormValid = false;
    } else {
    ...
    if (isLastNameValid == false) {
        ...
        document.getElementById("LastNameError").style.color = "rgb(168, 41, 56)";
        isFormValid = false;
    } else {
    ...
    if (isTelValid == false) {
        ...
        document.getElementById("telError").style.color = "rgb(199, 50, 68)";
        isFormValid = false;
    } else {
...
function signupOwner() {
    ...
    if (isLastNameOwnerValid == false) {
        document.getElementById("LastNameError").innerHTML = " ❌ LN must be alphabetic and 3+ charac long.";
        document.getElementById("LastNameError").style.color = "rgb(183, 68, 82)";
    } else {
        document.getElementById("LastNameError").innerHTML = "";
    }
    ...
    if (isFirstNameOwnerValid && isLastNameOwnerValid && isTelOwnerValid && isPwdOwnerValid && isEmailOwnerValid) {
        ...
        // Rediriger après avoir cliqué sur "Get Started"
        document.getElementById('getStartedBtn').addEventListener('click', function() {
            location.replace("login.html");
        });
    }
...
function login() {
    ...
    // Vérification de la correspondance avec un utilisateur
    for (let i = 0; i < usersTab.length; i++) {
        console.log("Checking user: ", usersTab[i]);  // Affichage de l'utilisateur en cours
...
function IsAlpha(name, nb) {
    var isValid = name.length >= nb;  // Vérifier la longueur d'abord

    var i = 0;
    // Vérifier chaque caractère de la chaîne
    while (i < name.length) {
        var char = name.charAt(i);  // Récupérer le caractère à l'index i
        if (!((char >= 'A' && char <= 'Z') || (char >= 'a' && char <= 'z'))) {  // Vérifier si c'est une lettre alphabétique
            isValid = false;
            break;
        }
        i++;
...
// Fonction pour afficher ou fermer le modal
...
function displayHouse() {
    ...
    // Parcourez housesTab pour générer le HTML dynamiquement
...
function displayRooms() {
        ...
        if (roomsTab[i].houseId == houseId) {
            ...
            content += `
                <article class="card__article">
                    <img src="images/gallery1.jpg" alt="Room Image" class="card__img">
                    ...
                    <div class="card__data">
                        ...
                        <button class="card__button" onclick="goToRoomDetails(${roomsTab[i].id})">Select Room</button>
                    </div>
                </article>`;
        }
    }
    ...
    // Afficher le contenu dans le container
...
function displayRoomDetails() {
    ...
    if (selectedRoom) {
        ...
        // Générer dynamiquement la liste des équipements avec des icônes Font Awesome
...
function editRoom(roomId) {
    ...
    // Form HTML structure with improved design
    var form = `
        <div class="row login_form" style="color: #000; background-color: #fff; padding: 20px; border-radius: 10px; border: 2px solid #000;">
...
function deleteRoom(roomId) {
    // Récupérer les données des chambres et des réservations depuis le localStorage
    var roomsTab = JSON.parse(localStorage.getItem("roomsTab") || "[]");
    var bookings = JSON.parse(localStorage.getItem("bookings") || "[]");

    // Trouver et supprimer la chambre
    var roomIndex = -1;
    for (let i = 0; i < roomsTab.length; i++) {
        if (roomsTab[i].id == roomId) {
            roomIndex = i;
    ...
    if (roomIndex !== -1) {
        ...
        localStorage.setItem("roomsTab", JSON.stringify(roomsTab)); // Mettre à jour le localStorage
    }
...
function deleteHouse(houseId) {
    // Supprimer toutes les chambres de cette maison
    let roomsTab = JSON.parse(localStorage.getItem("roomsTab") || "[]");
    for (let i = roomsTab.length - 1; i >= 0; i--) {
        if (roomsTab[i].houseId == houseId) {
            roomsTab.splice(i, 1); // Supprime la chambre
        }
    }
    localStorage.setItem("roomsTab", JSON.stringify(roomsTab));

    // Supprimer toutes les réservations associées à cette maison
    let bookings = JSON.parse(localStorage.getItem("bookings") || "[]");
    for (let i = bookings.length - 1; i >= 0; i--) {
        if (bookings[i].houseId == houseId) {
            bookings.splice(i, 1); // Supprime la réservation
        }
    }
    localStorage.setItem("bookings", JSON.stringify(bookings));

    // Supprimer la maison elle-même
    let housesTab = JSON.parse(localStorage.getItem("housesTab") || "[]");
    for (let i = housesTab.length - 1; i >= 0; i--) {
        if (housesTab[i].id == houseId) {
            housesTab.splice(i, 1); // Supprime la maison
...
function displayAdminUsers() {
    ...
    const housesTab = JSON.parse(localStorage.getItem("housesTab")) || []; // Récupérer le tableau des maisons
    let content = "";
...
function displayProfile() {
    ...
    if (!connectedUser) {
        ...
        // Génération dynamique du contenu avec icônes et titre rouge
        var content = `
            <div class="row">
                <div class="col-sm-6">
    ...
    var content = `
        <div class="row">
            <div class="col-sm-6">
                ...
                <span id="fnError" class="text-danger"></span> <!-- Erreur prénom -->
            </div>
            <div class="col-sm-6">
                <p class="font-weight-bold" style="font-size: 20px;">
                    <i class="fas fa-user"></i> Last Name:
        ...
        <div class="row mt-3">
            ...
            <div class="col-sm-6">
                ...
                <span id="phoneError" class="text-danger"></span> <!-- Erreur téléphone -->
            </div>
        </div>
        <div class="row mt-3">
            <div class="col-sm-12">
                ...
                <span id="addressError" class="text-danger"></span> <!-- Erreur adresse -->
            </div>
        </div>
